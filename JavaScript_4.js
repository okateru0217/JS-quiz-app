...
async function callApi() {
  ...
  const setUpQuiz = () => {
    ...
    genre.textContent = `【ジャンル】${quizData.results[quizNumber].category}`;
    difficultyLevel.textContent = `【難易度】${quizData.results[quizNumber].difficulty}`;
    question.textContent = quizData.results[quizNumber].question;
    
    innerAnswerButton.style.display = 'block';
    
    // 回答を配列に格納
    const quizDatas = quizData.results[quizNumber];
  ...
  for (let i = 0; i < answerButton.length; i++) {
    answerButton[i].addEventListener('click', () => {
      if (answerButton[i].textContent === quizData.results[quizNumber].correct_answer){
          score++;
        }
        clickHandler();
      });
  ...
  const clickHandler = () => {
    ...
    if (quizNumber < quizData.results.length) {
      setUpQuiz();
    } else {
